...
static void rio_reverse(char s[])
{
	int c, i, j;
...
void rio_itoa(int v, char s[], int b)
{
	int c, i = 0;
...
static size_t rio_strlen(char s[])
{
	int i = 0;
...
void Rio_puti(int fd, int v) /* Put long */
{
	char s[128];

	rio_itoa(v, s, 10); /* Based on K&R itoa() */  //line:csapp:sioltoa
	Rio_writen(fd, s, rio_strlen(s));
}
